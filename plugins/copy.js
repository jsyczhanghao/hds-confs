...
Copy.prototype.apply = function(compiler){
	...
	compiler.plugin("done", function(compilation){
        ...
        self.options.forEach(function(option){
            ...
            let include = option.include, exclude = option.exclude;

            if (include || exclude) {
                opts.filter = function (from) {
                    from = path.relative(compiler.context, from);
            ...
            fs.copy(
                ...
                console.log(chalk.blue('    ' + option.from + ' => ' + option.to + ' Complete!'));
            }).catch(function(){
                console.log(chalk.red('    ' + option.from + ' => ' + option.to + ' Failure!'));
            })
        });
	});
};
